...
beforeAll(() => {
  root = mkdtempSync(path.join(os.tmpdir(), 'ruqa-external-'))
...
describe('extractFilePaths', () => {
  it('keeps existing files and folders', () => {
    expect(extractFilePaths([filePath, dirPath], 'ruqa', null)).toEqual([filePath, dirPath])
  })
  ...
  it('drops flags, deep links and missing paths', () => {
    ...
    expect(extractFilePaths(args, 'ruqa', null)).toEqual([filePath])
  })
  ...
  it('never treats the app bundle itself as shared content', () => {
    ...
    expect(extractFilePaths(args, 'ruqa', dirPath)).toEqual([filePath])
  })
})
...
describe('readShareManifest', () => {
  ...
  it('reads the shared paths and removes the manifest', () => {
    ...
    expect(readShareManifest(`ruqa://share?manifest=${manifest}`, dirPath)).toEqual([
      filePath,
      '/tmp/other.jpg'
    ])
  ...
  it('ignores deep links that are not shares', () => {
    expect(readShareManifest('ruqa://join/abc', dirPath)).toEqual([])
    expect(readShareManifest('ruqa://share', dirPath)).toEqual([])
  ...
  it('drops non-string entries', () => {
    ...
    expect(readShareManifest(`ruqa://share?manifest=${manifest}`, dirPath)).toEqual([filePath])
  })
})
