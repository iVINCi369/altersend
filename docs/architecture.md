...
Ruqa is a monorepo with two apps (desktop, mobile) sharing five packages (core, drive, domain, components, locales). All P2P networking runs in a Bare worklet process, isolated from both the UI and the host runtime.
...
- `worklet/transfer/drive.ts` — the `@ruqa/drive` chunk channel over Protomux; the only transfer path
...
1. **Sender** opens the share screen → the worklet generates a single-use topic: a random 32-byte key, hex-encoded to a 64-char join code, displayed as QR / `com.ruqa.mobile://join/<topic>`.
...
6. Receiver requests each file over a `@ruqa/drive` chunk channel, streaming chunks straight to disk. Progress flows back over the control channel → RPC → domain → UI.

A peer that does not speak the drive protocol (Ruqa 1.6 or older) fails the download with an "update Ruqa" message. New transfer work belongs in `packages/drive`.
...
- **Caps** — a relay caps how much a session forwards. A sender raises its own cap with a short-lived token signed by the entitlement service and verified against the relay's public key: `relay/announce.ts` sends it over a protomux `ruqa-pro` channel, and `config.ts` releases it only while sending and only to a key from the signed relay list. A browser receiver asks its sender for the upgrade via the `web-relay` control message; `upgradeWebRelay.ts` dials the relay to announce it. Enforced relay-side.

The app is only ever a relay _client_ — it holds the relay's public key and address, never a secret.

## Browser receiver

`apps/web` is a receive-only client that runs in a plain browser — no install. It reuses the same transfer engine (`@ruqa/drive` over Protomux) and the [topic-authentication](#topic-authentication) handshake; the only thing that differs is how it reaches the DHT.
